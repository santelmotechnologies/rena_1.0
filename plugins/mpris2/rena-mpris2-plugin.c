...
RENA_PLUGIN_REGISTER (RENA_TYPE_MPRIS2_PLUGIN,
                        RenaMpris2Plugin,
                        rena_mpris2_plugin)
...
mpris_Root_Raise (GDBusMethodInvocation *invocation,
                  GVariant              *parameters,
                  RenaMpris2Plugin    *plugin)
{
	gtk_window_present(GTK_WINDOW(rena_application_get_window (plugin->priv->rena)));
...
mpris_Root_Quit (GDBusMethodInvocation *invocation,
                 ...
                 RenaMpris2Plugin    *plugin)
{
	rena_application_quit (plugin->priv->rena);

	g_dbus_method_invocation_return_value (invocation, NULL);
}

static GVariant *
mpris_Root_get_CanQuit (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_boolean(TRUE);
}

static GVariant *
mpris_Root_get_CanRaise (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_boolean(TRUE);
}

static GVariant *
mpris_Root_get_HasTrackList (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_boolean(TRUE);
}

static GVariant *
mpris_Root_get_Identity (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_string("Rena Music Player");
}

static GVariant *
mpris_Root_get_DesktopEntry (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_string("rena");
}

static GVariant *
mpris_Root_get_SupportedUriSchemes (GError **error, RenaMpris2Plugin *plugin)
{
...
static GVariant *
mpris_Root_get_SupportedMimeTypes (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_parse(G_VARIANT_TYPE("as"),
                   ...
                   RenaMpris2Plugin    *plugin)
{
	...
	backend = rena_application_get_backend (plugin->priv->rena);
	if (rena_backend_emitted_error (backend) == FALSE)
		rena_playback_play_pause_resume (plugin->priv->rena);
                   ...
                   RenaMpris2Plugin    *plugin)
{
	...
	backend = rena_application_get_backend (plugin->priv->rena);
	if (rena_backend_emitted_error (backend) == FALSE)
		rena_playback_next_track (plugin->priv->rena);
                       ...
                       RenaMpris2Plugin    *plugin)
{
	...
	backend = rena_application_get_backend (plugin->priv->rena);
	if (rena_backend_emitted_error (backend) == FALSE)
		rena_playback_prev_track (plugin->priv->rena);
                   ...
                   RenaMpris2Plugin    *plugin)
{
	...
	backend = rena_application_get_backend (plugin->priv->rena);
	if (rena_backend_emitted_error (backend) == FALSE)
		rena_playback_stop (plugin->priv->rena);
                   ...
                   RenaMpris2Plugin    *plugin)
{
	...
	gint64 curr_pos = rena_backend_get_current_position (backend) / GST_USECOND;
	gint64 seek = (curr_pos + param) / GST_MSECOND;

	seek = CLAMP (seek, 0, rena_musicobject_get_length (rena_backend_get_musicobject (backend)));
...
{
	RenaBackend *backend;
	RenaMusicobject *current_mobj = NULL;
	gint64 param;
	...
	if(mobj != NULL && mobj == current_mobj) {
		...
		if (seek >= rena_musicobject_get_length(current_mobj))
			seek = rena_musicobject_get_length(current_mobj);
...
static void
seeked_cb (RenaBackend *backend, gpointer user_data)
{
	RenaMpris2Plugin *plugin = user_data;
	...
	gint64 position = rena_backend_get_current_position (backend);
	...
	if(uri) {
		...
		if(path && is_playable_file(path)) {
			...
			if(mobj) {
				...
				rena_playlist_append_mobj_and_play(playlist, mobj);
				happened = TRUE;
			}
		}
		g_free(uri);
...
static GVariant *
mpris_Player_get_PlaybackStatus (GError **error, RenaMpris2Plugin *plugin)
{
	RenaBackend *backend = rena_application_get_backend (plugin->priv->rena);
...
static GVariant *
mpris_Player_get_LoopStatus (GError **error, RenaMpris2Plugin *plugin)
{
	RenaPreferences *preferences;
	gboolean repeat;

	preferences = rena_application_get_preferences (plugin->priv->rena);
	repeat = rena_preferences_get_repeat (preferences);
...
static GVariant *
mpris_Player_get_Rate (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_double(1.0);
}

static void
mpris_Player_put_Rate (GVariant *value, GError **error, RenaMpris2Plugin *plugin)
{
	g_set_error (error, G_DBUS_ERROR, G_DBUS_ERROR_NOT_SUPPORTED, "This is not alsaplayer.");
}

static GVariant *
mpris_Player_get_Shuffle (GError **error, RenaMpris2Plugin *plugin)
{
	RenaPreferences *preferences;
	gboolean shuffle;

	preferences = rena_application_get_preferences (plugin->priv->rena);
	shuffle = rena_preferences_get_shuffle (preferences);

	return g_variant_new_boolean(shuffle);
}

static void
mpris_Player_put_Shuffle (GVariant *value, GError **error, RenaMpris2Plugin *plugin)
{
	RenaPreferences *preferences;
...
static GVariant *
handle_get_trackid (RenaMusicobject *mobj)
{
	gchar *o = alloca(260);
	if(NULL == mobj)
...
static void
handle_get_metadata (RenaMusicobject *mobj, GVariantBuilder *b)
{
	...
	channels = rena_musicobject_get_channels(mobj);
	samplerate = rena_musicobject_get_samplerate(mobj);

	date = g_strdup_printf("%d", year);

	url = (rena_musicobject_is_local_file(mobj)) ?
...
static GVariant *
mpris_Player_get_Metadata (GError **error, RenaMpris2Plugin *plugin)
{
	RenaBackend *backend;
	RenaToolbar *toolbar;
	...
	if (rena_backend_get_state (backend) != ST_STOPPED) {
		handle_get_metadata(rena_backend_get_musicobject(backend), &b);

		toolbar = rena_application_get_toolbar (plugin->priv->rena);
		albumart = rena_toolbar_get_album_art (toolbar);

		arturl = rena_album_art_get_path(albumart);
...
static GVariant *
mpris_Player_get_Volume (GError **error, RenaMpris2Plugin *plugin)
{
	RenaBackend *backend = rena_application_get_backend (plugin->priv->rena);

	return g_variant_new_double(rena_backend_get_volume (backend));
}

static void
mpris_Player_put_Volume (GVariant *value, GError **error, RenaMpris2Plugin *plugin)
{
	RenaBackend *backend = rena_application_get_backend (plugin->priv->rena);
...
static GVariant *
mpris_Player_get_Position (GError **error, RenaMpris2Plugin *plugin)
{
	RenaBackend *backend = rena_application_get_backend (plugin->priv->rena);
	...
	if (rena_backend_get_state (backend) == ST_STOPPED)
		...
		return g_variant_new_int64(rena_backend_get_current_position(backend) / 1000);
}

static GVariant *
mpris_Player_get_MinimumRate (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_double(1.0);
}

static GVariant *
mpris_Player_get_MaximumRate (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_double(1.0);
}

static GVariant *
mpris_Player_get_CanGoNext (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_boolean(rena_playback_can_go_next(plugin->priv->rena));
}

static GVariant *
mpris_Player_get_CanGoPrevious (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_new_boolean(rena_playback_can_go_prev(plugin->priv->rena));
}

static GVariant *
mpris_Player_get_CanPlay (GError **error, RenaMpris2Plugin *plugin)
{
	RenaBackend *backend;
	gboolean can_play = FALSE;

	backend = rena_application_get_backend (plugin->priv->rena);
	can_play = ((rena_backend_get_state (backend) != ST_STOPPED) ||
	            (rena_playback_get_no_tracks(plugin->priv->rena) > 0));

	return g_variant_new_boolean (can_play);
}

static GVariant *
mpris_Player_get_CanPause (GError **error, RenaMpris2Plugin *plugin)
{
	RenaBackend *backend = rena_application_get_backend (plugin->priv->rena);

	return g_variant_new_boolean(rena_backend_get_state (backend) != ST_STOPPED);
}

static GVariant *
mpris_Player_get_CanSeek (GError **error, RenaMpris2Plugin *plugin)
{
	RenaBackend *backend = rena_application_get_backend (plugin->priv->rena);

	return g_variant_new_boolean (rena_backend_can_seek (backend));
}

static GVariant *
mpris_Player_get_CanControl (GError **error, RenaMpris2Plugin *plugin)
{
	// always?
	return g_variant_new_boolean(TRUE);
}
                                  ...
                                  RenaMpris2Plugin    *plugin)
{
	...
	if(get_playlist && g_str_has_prefix(get_playlist, MPRIS_PATH)) {
		cdbase = rena_application_get_database (plugin->priv->rena);
		db_playlists = rena_database_get_playlist_names (cdbase);
		if(db_playlists) {
	...
	if(found_playlist) {
		...
		add_playlist_current_playlist(found_playlist, plugin->priv->rena);
                              ...
                              RenaMpris2Plugin    *plugin)
{
	...
	cdbase = rena_application_get_database (plugin->priv->rena);
	lists = rena_database_get_playlist_names (cdbase);
...
static GVariant *
mpris_Playlists_get_ActivePlaylist (GError **error, RenaMpris2Plugin *plugin)
{
...
static GVariant *
mpris_Playlists_get_Orderings (GError **error, RenaMpris2Plugin *plugin)
{
	return g_variant_parse(G_VARIANT_TYPE("as"),
...
static GVariant *
mpris_Playlists_get_PlaylistCount (GError **error, RenaMpris2Plugin *plugin)
{
	RenaDatabase *cdbase;
	cdbase = rena_application_get_database (plugin->priv->rena);

	return g_variant_new_uint32 (rena_database_get_playlist_count (cdbase));
}
                      ...
                      RenaMpris2Plugin    *plugin)
{
	...
	if (mobj) {
		playlist = rena_application_get_playlist (plugin->priv->rena);
		rena_playlist_activate_unique_mobj (playlist, mobj);
		g_dbus_method_invocation_return_value (invocation, NULL);
	}
	else
		g_dbus_method_invocation_return_error_literal (invocation,
...
static GVariant *
mpris_TrackList_get_Tracks (GError **error, RenaMpris2Plugin *plugin)
{
	RenaPlaylist *playlist;
	GVariantBuilder builder;
	RenaMusicobject *mobj = NULL;
...
static GVariant *
mpris_TrackList_get_CanEditTracks (GError **error, RenaMpris2Plugin *plugin)
{
...
static void
rena_mpris_update_any (RenaMpris2Plugin *plugin)
{
	RenaBackend *backend;
	RenaBackendState playback_state = ST_STOPPED;
	RenaPreferences *preferences;
	...
	playback_state = rena_backend_get_state (backend);
	if (playback_state != ST_STOPPED) {
		current_title = rena_musicobject_get_file (rena_backend_get_musicobject (backend));
	}
	...
	curr_vol = rena_backend_get_volume (backend);
	...
	can_next = rena_playback_can_go_next (plugin->priv->rena);
	...
	can_prev = rena_playback_can_go_prev (plugin->priv->rena);
	...
	has_tracks = rena_playback_get_no_tracks(plugin->priv->rena);
	...
	can_seek = rena_backend_can_seek (backend);
...
static void
rena_mpris_update_metadata_changed (RenaMpris2Plugin *plugin)
{
	GVariantBuilder b;
...
static void
rena_mpris_update_mobj_remove(RenaMpris2Plugin *plugin, RenaMusicobject *mobj)
{
	GVariant * tuples[1];
...
static void
rena_mpris_update_mobj_added (RenaMpris2Plugin *plugin,
                                ...
                                GtkTreeIter        *iter)
{
	...
	playlist = rena_application_get_playlist (plugin->priv->rena);
	model = rena_playlist_get_model (playlist);
...
static void
rena_mpris_update_mobj_changed(RenaMpris2Plugin *plugin, RenaMusicobject *mobj, gint bitmask)
{
	GVariantBuilder b;
...
static void
rena_mpris_update_tracklist_replaced (RenaMpris2Plugin *plugin)
{
	RenaPlaylist *playlist;
	RenaBackend *backend;
	GVariantBuilder b;
	...
	g_variant_builder_close(&b);
	g_variant_builder_add_value(&b, handle_get_trackid(rena_backend_get_musicobject(backend)));
	g_dbus_connection_emit_signal (plugin->priv->dbus_connection, NULL, MPRIS_PATH,
...
any_notify_cb (GObject *gobject, GParamSpec *pspec, gpointer user_data)
{
	...
	rena_mpris_update_any (plugin);
}

static void
playlist_any_notify_cb (RenaPlaylist *playlist, gpointer user_data)
{
	RenaMpris2Plugin *plugin = user_data;
...
static void
rena_art_cache_changed_handler (RenaArtCache *cache, RenaMpris2Plugin *plugin)
{
	RenaBackend *backend;
	RenaMusicobject *mobj = NULL;
	...
	if (rena_backend_get_state (backend) != ST_STOPPED) {
		...
		if (album_art_path) {
			rena_mpris_update_metadata_changed (plugin);
...
rena_plugin_activate (PeasActivatable *activatable)
{
	...
	RenaMpris2Plugin *plugin = RENA_MPRIS2_PLUGIN (activatable);
	RenaMpris2PluginPrivate *priv = plugin->priv;
	...
	g_signal_connect (art_cache, "cache-changed",
	                  G_CALLBACK(rena_art_cache_changed_handler), plugin);
}
...
rena_plugin_deactivate (PeasActivatable *activatable)
{
	...
	art_cache = rena_application_get_art_cache (priv->rena);
	g_signal_handlers_disconnect_by_func (art_cache, rena_art_cache_changed_handler, plugin);
