...
static gchar *
get_playlist_dialog(RenaPlaylistActionRange type, GtkWidget *parent)
{
	GtkWidget *dialog;
	...
	if(type == SAVE_COMPLETE)
		rena_hig_workarea_table_add_section_title(table, &row, _("Save playlist"));
	else
		rena_hig_workarea_table_add_section_title(table, &row, _("Save selection"));
	...
	rena_hig_workarea_table_add_row(table, &row, label, entry);
...
gchar *
get_playlist_name(RenaPlaylistActionRange type, GtkWidget *parent)
{
	gchar *playlist = NULL;
...
save_mobj_list_to_m3u_playlist(GList *list, GIOChannel *chan, gchar *filename)
{
	...
	for (i=list; i != NULL; i = i->next) {
		...
		if (rena_musicobject_is_local_file(mobj)) {
			base = get_display_filename(rena_musicobject_get_file(mobj), TRUE);

			if (g_ascii_strcasecmp(base_m3u, base) == 0)
				uri = get_display_filename(rena_musicobject_get_file(mobj), FALSE);
			else
				uri = g_strdup(rena_musicobject_get_file(mobj));
...
#ifdef HAVE_PLPARSER
static void
rena_parser_append_foreach_playlist (GtkTreeModel *model,
                                       GtkTreePath  *path,
                                       GtkTreeIter  *iter,
                                       gpointer      data)
{
...
static gboolean
rena_parser_append_foreach_track_list (GtkTreeModel *model,
                                         GtkTreePath  *path,
                                         GtkTreeIter  *iter,
                                         gpointer      data)
{
	rena_parser_append_foreach_playlist (model,
		                                   path,
		                                   iter,
		                                   data);
	return FALSE;
...
static gboolean
rena_parser_save_full_track_list (RenaPlaylist *cplaylist,
                                    const gchar    *filename)
{
	...
	gtk_tree_model_foreach(rena_playlist_get_model(cplaylist),
	                       rena_parser_append_foreach_track_list,
	                       playlist);
...
static gboolean
rena_parser_save_selection_track_list (RenaPlaylist *cplaylist,
                                         const gchar    *filename)
{
	TotemPlPlaylist *playlist;
	TotemPlParser *pl;
	...
	selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(rena_playlist_get_view(cplaylist)));
	gtk_tree_selection_selected_foreach(selection,
	                                    rena_parser_append_foreach_playlist,
	                                    playlist);
...
#else
static gint
save_complete_m3u_playlist(RenaPlaylist* cplaylist, GIOChannel *chan, gchar *filename)
{
	gint ret = 0;
	GList *list = NULL;

	list = rena_playlist_get_mobj_list(cplaylist);
...
static gint
save_selected_to_m3u_playlist(RenaPlaylist* cplaylist, GIOChannel *chan, gchar *filename)
{
	GList *list = NULL;
	gint ret = 0;

	list = rena_playlist_get_selection_mobj_list(cplaylist);
...
gint
save_m3u_playlist(GIOChannel *chan, gchar *playlist, gchar *filename, RenaDatabase *cdbase)
{
	GList *list = NULL;
	gint ret = 0;
...
GList *
add_playlist_to_mobj_list(RenaDatabase *cdbase,
                          ...
                          GList *list)
{
	...
	playlist_id = rena_database_find_playlist (cdbase, playlist);
	...
	while (rena_prepared_statement_step (statement)) {
		const gchar *file = rena_prepared_statement_get_string (statement, 0);

		if ((location_id = rena_database_find_location (cdbase, file)))
...
GList *
add_radio_to_mobj_list(RenaDatabase *cdbase,
                       const gchar *radio,
                       GList *list)
{
	gint radio_id;
	...
	radio_id = rena_database_find_radio (cdbase, radio);
	...
	while (rena_prepared_statement_step (statement)) {
		const gchar *uri = rena_prepared_statement_get_string (statement, 0);
...
gchar* rename_playlist_dialog(const gchar *oplaylist, GtkWidget *parent)
{
	...
	table = rena_hig_workarea_table_new();
	rena_hig_workarea_table_add_section_title(table, &row, _("Choose a new name"));
...
void export_playlist (RenaPlaylist* cplaylist, RenaPlaylistActionRange choice)
{
	...
	switch(choice) {
	case SAVE_COMPLETE:
		if (rena_parser_save_full_track_list(cplaylist, filename) == FALSE)
			g_warning("Unable to save M3U playlist: %s", filename);
		break;
	case SAVE_SELECTED:
		if (rena_parser_save_selection_track_list(cplaylist, filename) == FALSE)
			g_warning("Unable to save M3U playlist: %s", filename);
		break;
	default:
		break;
...
GSList *
rena_totem_pl_parser_parse_from_uri (const gchar *uri)
{
...
#else
GSList *
rena_pl_parser_parse_xspf (const gchar *filename)
{
...
GSList *
rena_pl_parser_parse_pls (const gchar *file)
{
	GKeyFile *plskeyfile;
	GError *error = NULL;
	GSList *list = NULL;
...
GSList *
rena_pl_parser_parse_m3u (const gchar *file)
{
	GError *err = NULL;
	GIOChannel *chan = NULL;
	gint fd;
...
GSList *
rena_pl_parser_parse (RenaPlaylistType format, const gchar *filename)
{
	GSList *list = NULL;
	...
	{
	case PL_FORMAT_M3U:
		list = rena_pl_parser_parse_m3u (filename);
		break;
	case PL_FORMAT_PLS:
		list = rena_pl_parser_parse_pls (filename);
		break;
	case PL_FORMAT_ASX:
		//list = rena_pl_parser_parse_asx (filename);
		break;
	case PL_FORMAT_XSPF:
		list = rena_pl_parser_parse_xspf (filename);
		break;
	default:
	break;
    }
...
{
	RenaPlaylistType format = PL_FORMAT_UNKNOWN;
	GSList *list = NULL;

	if ((format = rena_pl_parser_guess_format_from_extension (filename)) != PL_FORMAT_UNKNOWN) {
		list = rena_pl_parser_parse (format, filename);
	}
	else {
...
rena_pl_parser_append_mobj_list_by_extension (GList *mlist, const gchar *file)
{
	...
	g_free (uri);
#else
	list = rena_pl_parser_parse_from_file_by_extension (file);
#endif
...
void rena_pl_parser_open_from_file_by_extension (const gchar *file, RenaApplication *rena)
{
	...
	notification = rena_app_notification_new (summary, NULL);
...
gchar *
rena_pl_get_first_playlist_item (const gchar *uri)
{
	gchar *file = NULL;
#ifdef HAVE_PLPARSER
	GSList *list = rena_totem_pl_parser_parse_from_uri(uri);
	if (list != NULL) {
...
void
save_playlist(RenaPlaylist* cplaylist,
              gint playlist_id,
              RenaPlaylistActionRange type)
{
	RenaDatabase *cdbase = NULL;
	GList *mlist = NULL, *i;
	const gchar *filename = NULL;

	switch(type) {
	case SAVE_COMPLETE:
		mlist = rena_playlist_get_mobj_list(cplaylist);
		break;
	case SAVE_SELECTED:
		mlist = rena_playlist_get_selection_mobj_list(cplaylist);
		break;
	default:
		break;
	}

	cdbase = rena_playlist_get_database (cplaylist);
	rena_database_begin_transaction (cdbase);
	if (mlist != NULL)
	{
		for (i = mlist; i != NULL; i = i->next)
		{
			filename = rena_musicobject_get_file (RENA_MUSICOBJECT(i->data));
			rena_database_add_playlist_track (cdbase, playlist_id, filename);
		}
		g_list_free(mlist);
	}
	rena_database_commit_transaction (cdbase);
}

void
new_playlist(RenaPlaylist* cplaylist,
             const gchar *playlist,
             RenaPlaylistActionRange type)
{
	gint playlist_id = 0;
	...
	if ((playlist_id = rena_database_find_playlist (rena_playlist_get_database(cplaylist), playlist))) {
		if (overwrite_existing_playlist(playlist, gtk_widget_get_toplevel(GTK_WIDGET(cplaylist))))
			rena_database_delete_playlist (rena_playlist_get_database(cplaylist), playlist);
		else
			return;
	}

	playlist_id = rena_database_add_new_playlist (rena_playlist_get_database(cplaylist), playlist);
...
void append_playlist(RenaPlaylist* cplaylist, const gchar *playlist, RenaPlaylistActionRange type)
{
	...
	playlist_id = rena_database_find_playlist (rena_playlist_get_database(cplaylist), playlist);
...
rena_playlist_database_update_playlist (RenaDatabase *cdbase, const gchar *playlist, GList *mlist)
{
	...
	if ((playlist_id = rena_database_find_playlist (cdbase, playlist)))
		rena_database_delete_playlist (cdbase, playlist);
	playlist_id = rena_database_add_new_playlist (cdbase, playlist);
...
rena_playlist_database_insert_playlist (RenaDatabase *cdbase, const gchar *playlist, GList *mlist)
{
	...
	if (mlist != NULL)
	{
		...
		{
			filename = rena_musicobject_get_file (RENA_MUSICOBJECT(i->data));
			if (!rena_database_playlist_has_track (cdbase, playlist_id, filename))
				rena_database_add_playlist_track (cdbase, playlist_id, filename);
		}
	}
	rena_database_commit_transaction (cdbase);
...
gchar *
new_radio (RenaPlaylist *playlist,
           const gchar    *uri,
           const gchar    *basename)
{
	...
	cdbase = rena_playlist_get_database(playlist);

	if (!rena_database_find_radio (cdbase, basename)) {
		...
		} while (rena_database_find_radio (cdbase, name));
	}

  	/* Save a new radio */

  	radio_id = rena_database_add_new_radio (cdbase, name);
	rena_database_add_radio_track (cdbase, radio_id, uri);

	return name;
}

RenaPlaylistAction
replace_or_append_dialog(RenaPlaylist *cplaylist, const gchar *playlist, RenaPlaylistActionRange type)
{
	GtkWidget *dialog;
	...
	gint result;
	guint row = 0;
	RenaPlaylistAction choise = EXPORT_PLAYLIST;

	/* Create dialog window */

	table = rena_hig_workarea_table_new();

	rena_hig_workarea_table_add_section_title(table, &row, _("What do you want to do?"));
	...
	rena_hig_workarea_table_add_wide_control(table, &row, radio_replace);
	...
	rena_hig_workarea_table_add_wide_control(table, &row, radio_add);
	g_free(string_options);
...
void
rena_playlist_save_selection (RenaPlaylist *playlist, const gchar *name)
{
	RenaPlaylistAction choise;
...
void
rena_playlist_save_playlist (RenaPlaylist *playlist, const gchar *name)
{
	RenaPlaylistAction choise;
...
static void
playlist_save_selection (GtkMenuItem *menuitem, RenaPlaylist *cplaylist)
{
	const gchar *playlist;
	playlist = gtk_menu_item_get_label (menuitem);
	rena_playlist_save_selection (cplaylist, playlist);
}

static void
playlist_save_complete_playlist (GtkMenuItem *menuitem, RenaPlaylist *cplaylist)
{
	const gchar *playlist;
	playlist = gtk_menu_item_get_label (menuitem);
	rena_playlist_save_playlist (cplaylist, playlist);
}

static void
update_playlist_changes_save_selection_popup_playlist (RenaPlaylist *cplaylist)
{
	GtkWidget *submenu, *menuitem;
	
	submenu = gtk_menu_new ();

	gtk_menu_item_set_submenu (GTK_MENU_ITEM (gtk_ui_manager_get_widget (rena_playlist_get_context_menu(cplaylist), "/SelectionPopup/Save selection")), submenu);
	...
	RenaPreparedStatement *statement = rena_database_create_statement (rena_playlist_get_database(cplaylist), sql);
	rena_prepared_statement_bind_string (statement, 1, SAVE_PLAYLIST_STATE);
	rena_prepared_statement_bind_string (statement, 2, _("Favorites"));
...
static void
update_playlist_changes_save_playlist_popup_playlist (RenaPlaylist *cplaylist)
{
	GtkWidget *submenu, *menuitem;
	
	submenu = gtk_menu_new ();

	gtk_menu_item_set_submenu (GTK_MENU_ITEM (gtk_ui_manager_get_widget (rena_playlist_get_context_menu(cplaylist), "/SelectionPopup/Save playlist")), submenu);
...
void
update_playlist_changes_on_menu (RenaPlaylist *playlist)
{
