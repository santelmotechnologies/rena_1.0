...
typedef struct _RenaEqualizerDialog RenaEqualizerDialog;

struct _RenaEqualizerDialog {
	GtkWidget         *enable;
...
{
	RenaEqualizerDialog *dialog = user_data;
	gint i, option = 0;
...
static void
rena_equalizer_dialog_init_bands (RenaEqualizerDialog *dialog)
{
	gchar *eq_preset = NULL;
	gdouble *saved_bands;
	gint i;

	eq_preset = rena_preferences_get_string (dialog->preferences,
	                                           GROUP_AUDIO,
	                                           KEY_EQ_PRESET);

	if(eq_preset != NULL) {
		if (g_ascii_strcasecmp(eq_preset, "Custom") == 0) {
			saved_bands = rena_preferences_get_double_list (dialog->preferences,
			                                                  GROUP_AUDIO,
			                                                  KEY_EQ_10_BANDS);
			if (saved_bands != NULL) {
				for (i = 0; i < NUM_BANDS; i++)
...
static void
rena_equalizer_dialog_save_preset (RenaEqualizerDialog *dialog)
{
	...
	rena_preferences_set_string(dialog->preferences,
	                              GROUP_AUDIO,
	                              KEY_EQ_PRESET,
	                              presets_names[preset]);

	rena_preferences_set_double_list(dialog->preferences,
	                                   GROUP_AUDIO,
	                                   KEY_EQ_10_BANDS,
	                                   bands,
	                                   NUM_BANDS);
...
static gboolean
rena_equalizer_band_get_tooltip (GtkWidget  *vscale,
                                   gint        x,
                                   gint        y,
...
static void
rena_equalizer_dialog_bind_bands_to_backend (RenaEqualizerDialog *dialog)
{
	gchar *eq_property;
...
static void
rena_equalizer_dialog_enabled_cb (GtkSwitch *enable,
									gpointer   data1,
                                    gpointer   data)
{
	RenaEqualizerDialog *dialog = data;
...
static void
rena_equalizer_dialog_response (GtkWidget *w_dialog,
                                  gint       response_id,
                                  gpointer   data)
{
	RenaEqualizerDialog *dialog = data;
	...
	if(dialog->equalizer != NULL)
		rena_equalizer_dialog_save_preset (dialog);

	g_object_unref (dialog->preferences);
	gtk_widget_destroy(w_dialog);

	g_slice_free(RenaEqualizerDialog, dialog);
}

void
rena_equalizer_dialog_show (RenaBackend *backend, GtkWidget *parent)
{
	RenaEqualizerDialog *dialog;
	GtkWidget *w_dialog, *grid, *label;
	GtkWidget *preamp_scale;
	gint i;

	dialog = g_slice_new0 (RenaEqualizerDialog);

	dialog->equalizer = rena_backend_get_equalizer (backend);
	dialog->preamp = rena_backend_get_preamp (backend);
	...
	g_signal_connect (dialog->enable, "notify::active",
	                  G_CALLBACK (rena_equalizer_dialog_enabled_cb), dialog);
	...
	g_signal_connect (G_OBJECT(preamp_scale), "query-tooltip",
	                  G_CALLBACK(rena_equalizer_band_get_tooltip),
	                  NULL);
	...
	if (dialog->equalizer != NULL && dialog->preamp != NULL) {
		rena_equalizer_dialog_bind_bands_to_backend (dialog);
		rena_equalizer_dialog_init_bands (dialog);
	}
	else {
	...
	g_signal_connect (G_OBJECT (w_dialog), "response",
	                  G_CALLBACK (rena_equalizer_dialog_response), dialog);
}
