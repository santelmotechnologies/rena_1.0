...
gchar *
rena_file_get_music_type(const gchar *filename)
{
	gchar *result = NULL;
...
RenaPlaylistType
rena_pl_parser_guess_format_from_extension (const gchar *filename)
{
...
/* Determine if the any file is useful to rena. */

RenaMediaType
rena_file_get_media_type (const gchar *filename)
{
	RenaMediaType ret = MEDIA_TYPE_UNKNOWN;
	gchar *result = NULL;
...
gboolean is_playable_file(const gchar *file)
{
	...
	if (g_file_test(file, G_FILE_TEST_IS_REGULAR) &&
	    (rena_file_get_media_type(file) != MEDIA_TYPE_AUDIO))
		return TRUE;
	else
		return FALSE;
}
...
gchar*
get_pref_image_path_dir (RenaPreferences *preferences, const gchar *path)
{
	GError *error = NULL;
	GDir *dir = NULL;
	const gchar *next_file = NULL;
	...
	patterns = rena_preferences_get_album_art_pattern(preferences);
...
gint rena_get_dir_count(const gchar *dir_name, GCancellable *cancellable)
{
	...
	while (next_file) {
		...
		if (g_file_test(ab_file, G_FILE_TEST_IS_DIR))
			file_count += rena_get_dir_count(ab_file, cancellable);
		else {
			file_count++;
		}
		g_free(ab_file);
		...
		if (is_dir_and_accessible(ab_file)) {
			...
			switch (file_type) {
				...
				case MEDIA_TYPE_PLAYLIST:
					list = rena_pl_parser_append_mobj_list_by_extension (list, ab_file);
					break;
...
append_mobj_list_from_unknown_filename(GList *list, gchar *filename)
{
	...
	if (is_dir_and_accessible(filename)) {
		...
		file_type = rena_file_get_media_type (filename);
		switch (file_type) {
			case MEDIA_TYPE_AUDIO:
				...
				break;
			case MEDIA_TYPE_PLAYLIST:
				list = rena_pl_parser_append_mobj_list_by_extension (list, filename);
				break;
			case MEDIA_TYPE_IMAGE:
			case MEDIA_TYPE_UNKNOWN:
			default:
