...
gchar *
rena_unescape_html_utf75 (const gchar *str)
{
	static GRegex *regex;
...
static gchar *
rena_string_strip_utf8 (const gchar *string)
{
	GString *gstring;
	gchar *normalized;
...
gchar *
rena_strstr_lv(gchar *haystack, gchar *needle, RenaPreferences *preferences)
{
	gboolean aproximate_search = FALSE;
	gchar *ret = NULL, *st_haystack = NULL, *st_needle = NULL;

	aproximate_search = rena_preferences_get_approximate_search(preferences);
	if (aproximate_search)
	{
		st_haystack = rena_string_strip_utf8 (haystack);
		st_needle = rena_string_strip_utf8 (needle);
...
GdkPixbuf *
rena_gdk_pixbuf_new_from_memory (gconstpointer data, gsize size)
{
	GError *error = NULL;
	...
	if (error) {
		g_warning ("rena_gdk_pixbuf_new_from_memory: %s\n", error->message);
		g_error_free (error);
	}
...
{
	return !rena_string_list_is_present (list, str);
}
...
rena_string_list_get_added (GSList *list, GSList *new_list)
{
	...
	for (i = new_list; i != NULL; i = i->next) {
		lstr = i->data;
		if (rena_string_list_is_not_present(list, lstr)) {
...
rena_string_list_get_removed (GSList *list, GSList *new_list)
{
	...
	for (i = list; i != NULL; i = i->next) {
		lstr = i->data;
		if (rena_string_list_is_not_present(new_list, lstr)) {
			ret_list = g_slist_append(ret_list, g_strdup(lstr));
		}
	}
	return ret_list;
...
gchar* get_display_name(RenaMusicobject *mobj)
{
	...
	if (!rena_musicobject_is_local_file(mobj)) {
		name = g_strdup(rena_musicobject_get_file(mobj));
	} else {
		name = get_display_filename(rena_musicobject_get_file(mobj), FALSE);
	}
	return name;
}
...
gchar *
rena_escape_slashes (const gchar *str)
{
...
{
#ifdef DEBUG
	extern GThread *rena_main_thread;
	if (g_thread_self () != rena_main_thread)
