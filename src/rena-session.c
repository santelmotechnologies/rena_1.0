...
#ifdef HAVE_LIBXFCE4UI
void
rena_init_session_support(RenaApplication *rena)
{
	XfceSMClient *client;
	GError *error = NULL;
	...
	g_signal_connect_swapped (G_OBJECT (client), "quit",
	                          G_CALLBACK (rena_application_quit), rena);
	...
	if(!xfce_sm_client_connect (client, &error)) {
		...
		g_object_set (GTK_APPLICATION(rena), "register-session", TRUE, NULL);
	}
}
#else
void
rena_init_session_support(RenaApplication *rena)
{
	...
	role = g_strdup_printf ("Rena-%p-%d-%d", window, (gint) getpid (), (gint) time (NULL));
