...
static void
rena_filter_dialog_select_row_on_current_playlist(GtkTreeView *fliter_view,
						    GtkTreePath *filter_path,
						    RenaFilterDialog *fdialog)
{
	GtkTreeIter iter;
	GtkTreeModel *filter_model;
	gint track_i = 0;
...
static int
rena_filter_dialog_key_press (GtkWidget *fliter_view,
				GdkEventKey *event,
				RenaFilterDialog *fdialog)
{
	...
	if (event->state != 0
		...
		if (list) {
			rena_filter_dialog_select_row_on_current_playlist (GTK_TREE_VIEW (fliter_view), list->data, fdialog);
			gtk_tree_path_free (list->data);
			g_list_free (list);

			rena_playlist_toggle_queue_selected (fdialog->cplaylist);
		}
		return TRUE;
	}
	return FALSE;
...
static gboolean
do_filter_dialog_refilter (RenaFilterDialog *fdialog)
{
	gtk_tree_model_filter_refilter (GTK_TREE_MODEL_FILTER(fdialog->filter_model));
...
static void
queue_filter_dialog_refilter (RenaFilterDialog *fdialog)
{
                                         ...
                                         RenaFilterDialog *fdialog)
{
	...
	if (!rena_preferences_get_instant_search(fdialog->preferences))
		return;
...
static gboolean
filter_model_visible_func (GtkTreeModel *model, GtkTreeIter *iter, RenaFilterDialog *fdialog)
{
	...
	approximate = rena_preferences_get_approximate_search(fdialog->preferences);
...
static void
rena_filter_dialog_fill_model (GtkListStore *filter_model, RenaPlaylist *cplaylist)
{
	...
	track_i = rena_playlist_get_no_tracks(cplaylist);
...
static void
rena_filter_dialog_activated_cb(GtkTreeView *fliter_view,
				  GtkTreePath *filter_path,
...
static void
rena_filter_dialog_response(GtkDialog *dialog,
			      gint response,
			      RenaFilterDialog *fdialog)
{
	GList *list;
	...
	case GTK_RESPONSE_ACCEPT:
		...
		list = gtk_tree_selection_get_selected_rows (selection, NULL);
		if (list) {
			rena_filter_dialog_select_row_on_current_playlist (GTK_TREE_VIEW (fdialog->filter_view), list->data, fdialog);
			gtk_tree_path_free (list->data);
			g_list_free (list);
		}
		rena_playlist_toggle_queue_selected (fdialog->cplaylist);
	...
	gtk_widget_grab_focus (rena_playlist_get_view (fdialog->cplaylist));
	gtk_widget_destroy (GTK_WIDGET(dialog));

	g_free(fdialog->filter_string);
	g_object_unref(G_OBJECT(fdialog->preferences));
	g_slice_free(RenaFilterDialog, fdialog);
}

void
rena_filter_dialog (RenaPlaylist *playlist)
{
	RenaPreferences *preferences;
	...
	RenaFilterDialog *fdialog;
	fdialog = g_slice_new0(RenaFilterDialog);

	preferences = rena_preferences_get ();

	/* Crete the filter entry */

	search_entry = rena_search_entry_new(preferences);
	...
	rena_filter_dialog_fill_model (filter_store, playlist);
	...
	g_signal_connect (filter_view, "row-activated",
			G_CALLBACK(rena_filter_dialog_activated_cb), dialog);
	g_signal_connect (filter_view, "key_press_event",
			  G_CALLBACK (rena_filter_dialog_key_press), fdialog);

	g_signal_connect(G_OBJECT(dialog), "response",
			G_CALLBACK(rena_filter_dialog_response), fdialog);
