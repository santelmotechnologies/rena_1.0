...
static void
seach_entry_populate_popup (GtkEntry *entry, RenaPreferences *preferences)
{
	...
	gtk_menu_popup(GTK_MENU(popup_menu), NULL, NULL,
			(GtkMenuPositionFunc) rena_utils_set_menu_position, entry,
			0, gtk_get_current_event_time());
}

static void
rena_search_bar_icon_pressed_cb (GtkEntry       *entry,
                                   gint            position,
                                   ...
                                   RenaPreferences *preferences)
{
	if (position == GTK_ENTRY_ICON_SECONDARY) {
		...
		if (!rena_preferences_get_instant_search(preferences))
...
GtkWidget *
rena_search_entry_new (RenaPreferences *preferences)
{
	...
	g_signal_connect (search_entry, "icon-press", G_CALLBACK (rena_search_bar_icon_pressed_cb), preferences);
